...
export default function EditProblemPage({ params }) {
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Error fetching problem: ${response.statusText}`);
        }
        
        const data = await response.json();
        
        if (data.success && data.problem) {
          // Ensure templateCode is properly structured
          let templateCode = { ...DEFAULT_TEMPLATE_CODE };
          if (data.problem.templateCode && typeof data.problem.templateCode === 'object') {
            templateCode = {
              ...DEFAULT_TEMPLATE_CODE,
              ...data.problem.templateCode
            };
          }
          
          // Format test cases properly
          const testCases = data.problem.testCases.length > 0 
          ...
          setFormData({
            ...
            exampleInput: data.problem.exampleInput || '',
            exampleOutput: data.problem.exampleOutput || '',
            constraints: data.problem.constraints || '',
            solution: data.problem.solution || '',
            timeComplexity: data.problem.timeComplexity || 'O(n)',
            spaceComplexity: data.problem.spaceComplexity || 'O(n)',
            categories: data.problem.categories || [],
            templateCode,
            testCases,
  ...
  // Form submission for updating the problem
    ...
    try {
      ...
      const dataToSubmit = {
        ...
        testCases: formData.testCases.map(tc => ({
          id: tc.id, // Include ID if it exists
          input: tc.input.trim(),
      ...
      console.log("Submitting update data:", JSON.stringify(dataToSubmit, null, 2));
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh]">
        ...
        <p className="text-lg text-gray-600 dark:text-gray-300">Loading problem data...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      {error && (
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900 rounded-lg p-4 flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-medium text-red-800 dark:text-red-300">Error updating problem</h3>
      ...
      {success && (
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-900 rounded-lg p-4 flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-medium text-green-800 dark:text-green-300">Problem updated successfully</h3>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-10">
                    {formData.testCases.map((testCase, index) => (
                      <div key={index} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6 relative border border-gray-200 dark:border-gray-600 shadow-sm">
